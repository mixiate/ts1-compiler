...
fn create_palt_chunk(palette_id: iff::ChunkId, sprite_path: &std::path::Path) -> (u8, Vec<u8>) {
    ...
    assert!(palette.len() == PALT_COLOUR_ENTRY_COUNT as usize);
    const TRANSPARENT_COLOUR: [u8; 3] = [255, 255, 0];
    let transparent_colour_index =
        u8::try_from(palette.iter().position(|entry| *entry == TRANSPARENT_COLOUR).unwrap()).unwrap();
    ...
    (transparent_colour_index, palt_chunk)
}
...
pub fn create_palt_chunks(
    ...
    let mut palt_transparent_colour_indexes = std::collections::HashMap::new();
    let mut palt_chunks = Vec::new();

    const INVALID_TRANSPARENT_COLOUR_INDEX: u8 = 255;
    palt_transparent_colour_indexes
        .entry(iff::ChunkId::invalid())
        .or_insert_with(|| INVALID_TRANSPARENT_COLOUR_INDEX);

    for sprite in sprites {
        palt_transparent_colour_indexes.entry(sprite.palette_chunk_id).or_insert_with(|| {
            ...
            let (transparent_colour_index, palt_chunk) =
    ...
    (palt_transparent_colour_indexes, palt_chunks)
}
